...
export class SleepTracking {
    ...
    static create(
        ...
        if (typeof value !== "number" || !Number.isFinite(value)) {
            throw new Error("Sleep tracking value must be finite.");
        }

        const effectiveRecordedAt = recordedAt ?? new Date();

        if (Number.isNaN(effectiveRecordedAt.getTime())) {
            throw new Error("Sleep tracking date is invalid.");
        }

        if ([sourceType, sourceId, sourceObservationId].some(v => !v?.trim())) {
            throw new Error("Complete sleep tracking provenance is required.");
        }
