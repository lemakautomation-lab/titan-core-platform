...
export class RestTracking {
    ...
    static create(
        ...
        if (typeof value !== "number" || !Number.isFinite(value)) {
            throw new Error("Rest tracking value must be finite.");
        }
        ...
        if (Number.isNaN(effectiveRecordedAt.getTime())) {
            throw new Error("Rest tracking date is invalid.");
        }

        if (
            [sourceType, sourceId, sourceObservationId]
