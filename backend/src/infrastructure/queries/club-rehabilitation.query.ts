import { ClubRehabilitationReader } from "../../application/use-cases/list-club-rehabilitation.use-case";
import { DatabaseService } from "../database/database.service";

export class PrismaClubRehabilitationReader implements ClubRehabilitationReader {
    ...
    async read(tenantId: string, userId: string, limit: number, cursor: string | null) {
        ...
        const where = {
            ...
            userRoles: { some: { role: { tenantId, name: "REHABILITATION_PROFESSIONAL" } } },
        };
        if (cursor) {
        ...
        return {
            organisationId: club.id,
            rehabilitation: page.map((user) => ({
                id: user.id,
                name: [user.firstName, user.lastName].filter(Boolean).join(" ") || "Rehabilitation Professional",
            })),
