import { ClubConditioningReader } from "../../application/use-cases/list-club-conditioning.use-case";
import { DatabaseService } from "../database/database.service";

export class PrismaClubConditioningReader implements ClubConditioningReader {
    ...
    async read(tenantId: string, userId: string, limit: number, cursor: string | null) {
        ...
        const where = {
            ...
            userRoles: { some: { role: { tenantId, name: "STRENGTH_CONDITIONING" } } },
        };
        if (cursor) {
        ...
        return {
            organisationId: club.id,
            conditioning: page.map((user) => ({
                id: user.id,
                name: [user.firstName, user.lastName].filter(Boolean).join(" ") || "Strength & Conditioning Professional",
            })),
