import { ClubDirectorReader } from "../../application/use-cases/list-club-directors.use-case";
import { DatabaseService } from "../database/database.service";

export class PrismaClubDirectorReader implements ClubDirectorReader {
    ...
    async read(tenantId: string, userId: string, limit: number, cursor: string | null) {
        ...
        const where = {
            ...
            userRoles: { some: { role: { tenantId, name: "PERFORMANCE_DIRECTOR" } } },
        };
        if (cursor) {
        ...
        return {
            organisationId: club.id,
            directors: page.map((user) => ({
                id: user.id,
                name: [user.firstName, user.lastName].filter(Boolean).join(" ") || "Performance Director",
            })),
