import { ClubNutritionReader } from "../../application/use-cases/list-club-nutrition.use-case";
import { DatabaseService } from "../database/database.service";

export class PrismaClubNutritionReader implements ClubNutritionReader {
    ...
    async read(tenantId: string, userId: string, limit: number, cursor: string | null) {
        ...
        const where = {
            ...
            userRoles: { some: { role: { tenantId, name: "NUTRITION_PROFESSIONAL" } } },
        };
        if (cursor) {
        ...
        return {
            organisationId: club.id,
            nutrition: page.map((user) => ({
                id: user.id,
                name: [user.firstName, user.lastName].filter(Boolean).join(" ") || "Nutrition Professional",
            })),
