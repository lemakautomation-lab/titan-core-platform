...
implements WorkoutProgrammeGenerationTransaction {
    ...
    async execute(
        ...
        return this.executeAttempt(input);
    }
    ...
    private async executeAttempt(
        ...
        try {
            return await this.database.prisma.$transaction(async tx => {
                ...
                await tx.workoutProgrammeGeneration.create({
                    data: {
                        ...
                        inputSnapshot:
                            generation.inputSnapshot as Prisma.InputJsonValue,
                        planSnapshot:
                            generation.planSnapshot as Prisma.InputJsonValue,
            ...
            if (this.isIdempotencyUniqueConstraintConflict(error)) {
                return this.loadCommittedReplay(input);
            }

            if (this.isRetryableTransactionConflict(error)) {
                const existing =
                    await this.database.prisma.workoutProgrammeGeneration.findUnique({
                        where: {
                            tenantId_idempotencyKey: {
                                tenantId: input.tenantId,
                                idempotencyKey: input.idempotencyKey,
    ...
    private async loadCommittedReplay(
        input: WorkoutProgrammeGenerationTransactionInput,
    ): Promise<WorkoutProgrammeGenerationTransactionOutcome> {
        const existing =
            await this.database.prisma.workoutProgrammeGeneration.findUnique({
        ...
        if (!existing) {
            throw new Error("Idempotency conflict could not be resolved.");
        }
    ...
    private advisoryKey(
        ...
        const digest = createHash("sha256")
            .update(
                `${PrismaWorkoutProgrammeGenerationTransaction.LOCK_VERSION}\u0000${tenantId}\u0000${idempotencyKey}`,
                "utf8",
            )
            .digest();
    ...
    private isIdempotencyUniqueConstraintConflict(
        error: unknown,
    ): boolean {
        if (
            !(error instanceof Prisma.PrismaClientKnownRequestError) ||
