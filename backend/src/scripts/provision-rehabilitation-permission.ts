...
const permissionCode = "performance-professional.rehabilitation.read";
...
async function main(): Promise<void> {
    ...
    if (!role) {
        throw new Error("Role not found in the specified tenant.");
    }
    ...
    await prisma.$transaction(async (tx) => {
        const permission = await tx.permission.upsert({
            ...
            create: {
                ...
                code: permissionCode,
                name: permissionCode,
                description: "Read linked athlete rehabilitation workflow",
            },
        });
        ...
        await tx.auditLog.create({
            data: {
                tenantId,
                action: "REHABILITATION_PERMISSION_PROVISION",
