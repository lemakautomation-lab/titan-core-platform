-- Dedicated cross-team visibility grant; existing tenant administrators only.
INSERT INTO "Permission" ("id", "tenantId", "code", "name", "description", "createdAt", "updatedAt")
SELECT gen_random_uuid()::text, t."id",
       'performance-director.teams.read', 'performance-director.teams.read',
       'List active teams in own department', NOW(), NOW()
FROM "Tenant" AS t
WHERE NOT EXISTS (
    SELECT 1 FROM "Permission" AS p
    WHERE p."tenantId" = t."id" AND p."code" = 'performance-director.teams.read'
)
...
JOIN "Permission" AS p ON p."tenantId" = r."tenantId"
 AND p."code" = 'performance-director.teams.read'
WHERE r."name" = 'ADMIN'
