-- Dedicated club nutrition roster permission for existing tenant administrators.
INSERT INTO "Permission" ("id", "tenantId", "code", "name", "description", "createdAt", "updatedAt")
SELECT gen_random_uuid()::text, t."id",
       'club.nutrition.read',
       'club.nutrition.read',
       'Read own club nutrition roster', NOW(), NOW()
FROM "Tenant" AS t
WHERE NOT EXISTS (
    SELECT 1 FROM "Permission" AS p
    WHERE p."tenantId" = t."id" AND p."code" = 'club.nutrition.read'
)
...
JOIN "Permission" AS p ON p."tenantId" = r."tenantId"
 AND p."code" = 'club.nutrition.read'
WHERE r."name" = 'ADMIN'
