-- Dedicated club conditioning roster permission for existing tenant administrators.
INSERT INTO "Permission" ("id", "tenantId", "code", "name", "description", "createdAt", "updatedAt")
SELECT gen_random_uuid()::text, t."id",
       'club.conditioning.read',
       'club.conditioning.read',
       'Read own club strength and conditioning roster', NOW(), NOW()
FROM "Tenant" AS t
WHERE NOT EXISTS (
    SELECT 1 FROM "Permission" AS p
    WHERE p."tenantId" = t."id" AND p."code" = 'club.conditioning.read'
)
...
JOIN "Permission" AS p ON p."tenantId" = r."tenantId"
 AND p."code" = 'club.conditioning.read'
WHERE r."name" = 'ADMIN'
