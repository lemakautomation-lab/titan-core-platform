...
import {
    CreateTrainingStressUseCase,
    CreateTrainingStressCommand,
} from "../../src/application/use-cases/create-training-stress.use-case";

import {
    ListRecentTrainingStressUseCase,
    ListRecentTrainingStressQuery,
} from "../../src/application/use-cases/list-recent-training-stress.use-case";
...
import { TrainingStressRepository } from "../../src/domain/repositories/training-stress.repository";
...
function createTrackingRepository(
    ...
    } as unknown as TrainingStressRepository;
}

describe(
    "Training Stress Application Boundary",
    () => {

        it(
            "creates training stress for a valid athlete",
            async () => {
                const repository =
                    createTrackingRepository();

                const useCase =
                    new CreateTrainingStressUseCase(
                        repository,
                ...
                const command:
                    CreateTrainingStressCommand = {
                        ...
                        sourceObservationId: "training-stress-1",
                    };
        ...
        it(
            "rejects invalid training stress values",
            async () => {
                const repository =
                    createTrackingRepository();
                ...
                expect(result.error)
                    .toBe(
                        "Training stress value must be finite.",
                    );
        ...
        it(
            ...
            async () => {
                ...
                expect(result.error)
                    .toBe(
                        "Complete training stress provenance is required.",
                    );
        ...
        it(
            "rejects a non-positive recent-training-stress limit",
            async () => {
                const repository =
                    createTrackingRepository();

                const useCase =
                    new ListRecentTrainingStressUseCase(
                        repository,
                        createAthleteRepository(),
                    );
                ...
                expect(result.error)
                    .toBe(
                        "Training stress limit must be positive.",
                    );
        ...
        it(
            "rejects a recent-training-stress limit above 100",
            async () => {
                const repository =
                    createTrackingRepository();
