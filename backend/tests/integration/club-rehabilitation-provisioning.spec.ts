...
const code = "club.rehabilitation.read";

describe("Mission 070.7 new-tenant permission provisioning", () => {
    it("dry runs, rejects foreign roles and applies an idempotent audited grant", async () => {
        const tenant = await testPrisma.tenant.create({ data: {
            name: "070.7 provisioning gate", slug: `gate-0707-${randomUUID()}`,
        } });
        const foreignTenant = await testPrisma.tenant.create({ data: {
            name: "070.7 foreign tenant", slug: `gate-0707-${randomUUID()}`,
        } });
        try {
            ...
            const args = [
                "dist/scripts/provision-club-rehabilitation-permission.js",
            ...
            expect(await testPrisma.auditLog.count({ where: {
                tenantId: tenant.id, action: "CLUB_REHABILITATION_PERMISSION_PROVISION",
            } })).toBe(2);
        } finally {
