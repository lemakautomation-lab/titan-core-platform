...
describe("Mission 066.2 Coach team API", () => {
it("requires authentication", async () => {
    ...
    it("creates, lists and updates only the authenticated coach's team", async () => {
        const {
            user,
            password,
        ...
        try {
            ...
            const persisted =
                await testPrisma.coachTeam.findUnique({
                    where: {
                        id: teamId,
                    },
                });
    ...
    it("validates team details", async () => {
        const {
            user,
            password,
        } = await createAuthorizedCoach();
    ...
    it("enforces tenant isolation", async () => {
        ...
        try {
            ...
            const foreignUpdate =
                await request(app)
                    .patch(
                        `/api/v1/coach/teams/${created.body.id}`,
                    )
                    .set(
                        "Authorization",
                        `Bearer ${secondToken}`,
