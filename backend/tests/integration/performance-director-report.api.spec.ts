...
const endpoint = "/api/v1/performance-director/report";
const permission = "performance-director.reports.read";
...
describe("Mission 069.4 role-specific department report", () => {
    beforeEach(async () => { await rateLimitModule.resetAuthRateLimiter(); });

    it("requires authentication and its own reporting permission", async () => {
        expect((await request(app).get(endpoint)).status).toBe(401);
        const director = await createTestUser({
            permissions: ["performance-director.intelligence.read"],
        });
    ...
    it("reports only aggregate observations for active athletes in the assigned department", async () => {
        const director = await createTestUser({ permissions: [permission] });
        ...
        expect(response.body).toEqual({
            organisationId: own.id, organisationName: own.name, days: 7, staffCount: 1, activeAthleteCount: 1,
