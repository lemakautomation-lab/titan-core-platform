...
const code = "club.scientists.read";

describe("Mission 070.4 new-tenant permission provisioning", () => {
    it("dry runs, rejects foreign roles and applies an idempotent audited grant", async () => {
        const tenant = await testPrisma.tenant.create({ data: {
            name: "069.5 provisioning gate", slug: `gate-0704-${randomUUID()}`,
        } });
        const foreignTenant = await testPrisma.tenant.create({ data: {
            name: "069.5 foreign tenant", slug: `gate-0704-${randomUUID()}`,
        } });
        try {
            ...
            const args = [
                "dist/scripts/provision-club-scientists-permission.js",
            ...
            expect(await testPrisma.auditLog.count({ where: {
                tenantId: tenant.id, action: "CLUB_SCIENTISTS_PERMISSION_PROVISION",
            } })).toBe(2);
        } finally {
