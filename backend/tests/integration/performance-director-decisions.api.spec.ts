...
const endpoint = "/api/v1/performance-director/decision-support";
const permission = "performance-director.decisions.read";
...
describe("Mission 069.5 department decision support", () => {
    beforeEach(async () => { await rateLimitModule.resetAuthRateLimiter(); });

    it("requires authentication and its own decision permission", async () => {
        expect((await request(app).get(endpoint)).status).toBe(401);
        const director = await createTestUser({
            permissions: ["performance-director.reports.read"],
        });
    ...
    it("returns bounded review action for active athletes in the assigned department", async () => {
        const director = await createTestUser({ permissions: [permission] });
