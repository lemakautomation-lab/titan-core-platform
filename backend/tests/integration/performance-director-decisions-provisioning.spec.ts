...
const code = "performance-director.decisions.read";

describe("Mission 069.5 new-tenant permission provisioning", () => {
    it("dry runs, rejects foreign roles and applies an idempotent audited grant", async () => {
        const tenant = await testPrisma.tenant.create({ data: {
            name: "069.5 provisioning gate", slug: `gate-0695-${randomUUID()}`,
        } });
        const foreignTenant = await testPrisma.tenant.create({ data: {
            name: "069.5 foreign tenant", slug: `gate-0695-${randomUUID()}`,
        } });
        try {
            ...
            const args = [
                "dist/scripts/provision-director-decisions-permission.js",
            ...
            expect(await testPrisma.auditLog.count({ where: {
                tenantId: tenant.id, action: "DIRECTOR_DECISIONS_PERMISSION_PROVISION",
            } })).toBe(2);
        } finally {
