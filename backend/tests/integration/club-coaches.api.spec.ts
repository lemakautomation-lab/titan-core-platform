...
const endpoint = "/api/v1/club/coaches";
const permission = "club.coaches.read";
...
describe("Mission 070.3 club coaches", () => {
    beforeEach(async () => { await rateLimitModule.resetAuthRateLimiter(); });

    it("requires authentication and the dedicated coach permission", async () => {
    ...
    it("pages only explicitly assigned active coaches in the actor's active club", async () => {
        const actor = await createTestUser({ permissions: [permission] });
        ...
        const role = await testPrisma.role.create({ data: { tenantId, name: "COACH" } });
        const createCoach = async (organisationId: string, status: "ACTIVE" | "INACTIVE" = "ACTIVE") => {
            const identity = await createTestUser({ tenantId });
            await testPrisma.user.update({ where: { id: identity.user.id },
                data: { organisationId, status, firstName: "Named", lastName: "Coach" },
            });
            await testPrisma.userRole.create({ data: { userId: identity.user.id, roleId: role.id } });
            return identity.user.id;
        };
        const firstId = await createCoach(own.id);
        const secondId = await createCoach(own.id);
        const otherId = await createCoach(other.id);
        await createCoach(own.id, "INACTIVE");
        ...
        expect(first.body).toEqual({ organisationId: own.id,
            coaches: [{ id: ordered[0], name: "Named Coach" }], nextCursor: ordered[0],
        });
        const second = await fetchPage(first.body.nextCursor);
        expect(second.body.coaches).toEqual([{ id: ordered[1], name: "Named Coach" }]);
