...
const code = "performance-director.intelligence.read";

describe("Mission 069.2 new-tenant permission provisioning", () => {
    it("dry runs, rejects foreign roles and applies an idempotent audited grant", async () => {
        const tenant = await testPrisma.tenant.create({ data: {
            name: "069.2 provisioning gate", slug: `gate-0692-${randomUUID()}`,
        } });
        const foreignTenant = await testPrisma.tenant.create({ data: {
            name: "069.2 foreign tenant", slug: `gate-0692-${randomUUID()}`,
        } });
        try {
            ...
            const args = [
                "dist/scripts/provision-director-intelligence-permission.js",
            ...
            expect(await testPrisma.auditLog.count({ where: {
                tenantId: tenant.id, action: "DIRECTOR_INTELLIGENCE_PERMISSION_PROVISION",
            } })).toBe(2);
        } finally {
