...
const code = "performance-director.command-centre.read";

describe("Mission 069.1 new-tenant permission provisioning", () => {
    it("dry runs, rejects foreign roles and applies an idempotent audited grant", async () => {
        const tenant = await testPrisma.tenant.create({ data: {
            name: "069.1 provisioning gate", slug: `gate-0691-${randomUUID()}`,
        } });
        const foreignTenant = await testPrisma.tenant.create({ data: {
            name: "069.1 foreign tenant", slug: `gate-0691-${randomUUID()}`,
        } });
        try {
            ...
            const args = [
                "dist/scripts/provision-director-command-centre-permission.js",
            ...
            expect(await testPrisma.auditLog.count({ where: {
                tenantId: tenant.id, action: "DIRECTOR_COMMAND_CENTRE_PERMISSION_PROVISION",
            } })).toBe(2);
        } finally {
