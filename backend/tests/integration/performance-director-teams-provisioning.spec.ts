...
const code = "performance-director.teams.read";

describe("Mission 069.3 new-tenant permission provisioning", () => {
    it("dry runs, rejects foreign roles and applies an idempotent audited grant", async () => {
        const tenant = await testPrisma.tenant.create({ data: {
            name: "069.3 provisioning gate", slug: `gate-0693-${randomUUID()}`,
        } });
        const foreignTenant = await testPrisma.tenant.create({ data: {
            name: "069.3 foreign tenant", slug: `gate-0693-${randomUUID()}`,
        } });
        try {
            ...
            const args = [
                "dist/scripts/provision-director-teams-permission.js",
            ...
            expect(await testPrisma.auditLog.count({ where: {
                tenantId: tenant.id, action: "DIRECTOR_TEAMS_PERMISSION_PROVISION",
            } })).toBe(2);
        } finally {
