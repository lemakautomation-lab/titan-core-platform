...
const endpoint = "/api/v1/club/executives";
const permission = "club.executives.read";
...
describe("Mission 070.1 club executive structure", () => {
    ...
    it("requires authentication and the dedicated club permission", async () => {
        expect((await request(app).get(endpoint)).status).toBe(401);
        const actor = await createTestUser({ permissions: ["performance-director.command-centre.read"] });
    ...
    it("pages only explicitly assigned active executives in the actor's active club", async () => {
        const actor = await createTestUser({ permissions: [permission] });
        ...
        await testPrisma.organisation.create({ data: {
            tenantId, parentOrganisationId: own.id, name: "Active unit", slug: `unit-${randomUUID()}`,
        } });
        await testPrisma.organisation.create({ data: {
            tenantId, parentOrganisationId: own.id, status: "INACTIVE",
            name: "Inactive unit", slug: `unit-${randomUUID()}`,
        } });
        const role = await testPrisma.role.create({ data: { tenantId, name: "CLUB_EXECUTIVE" } });
        const createExecutive = async (organisationId: string, status: "ACTIVE" | "INACTIVE" = "ACTIVE") => {
            const identity = await createTestUser({ tenantId });
            await testPrisma.user.update({ where: { id: identity.user.id },
                data: { organisationId, status, firstName: "Named", lastName: "Executive" },
            });
            await testPrisma.userRole.create({ data: { userId: identity.user.id, roleId: role.id } });
            return identity.user.id;
        };
        const firstId = await createExecutive(own.id);
        const secondId = await createExecutive(own.id);
        const otherId = await createExecutive(other.id);
        await createExecutive(own.id, "INACTIVE");
        ...
        expect(first.body).toEqual({ organisationId: own.id, organisationName: own.name,
            activeChildOrganisationCount: 1,
            executives: [{ id: ordered[0], name: "Named Executive" }], nextCursor: ordered[0],
        });
        const second = await fetchPage(first.body.nextCursor);
        expect(second.body.executives).toEqual([{ id: ordered[1], name: "Named Executive" }]);
