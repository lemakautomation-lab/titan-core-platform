...
const endpoint = "/api/v1/club/teams";
const permission = "club.teams.read";
...
describe("Mission 070.8 club teams", () => {
    ...
    it("validates the page request and hides absent or unassigned clubs", async () => {
        ...
        expect(response.body).toEqual({ error: "Club page not found." });
    });
    ...
    it("pages active teams across coaches in the same organisation only", async () => {
        ...
        for (const user of [actor, coachA, coachB, inactiveCoach]) {
        ...
        expect(foreignCursor.body).toEqual({ error: "Club page not found." });
    });
});
