...
import { PrismaRestTrackingRepository } from "../../src/infrastructure/repositories/rest-tracking/rest-tracking.repository";
...
const repository =
    new PrismaRestTrackingRepository(
        database,
    );
    ...
    async () => {
        const tenant =
            await testPrisma.tenant.create({
                data: {
                    name:
                        `TITAN Rest Test Tenant ${crypto.randomUUID()}`,
                    slug:
                        `titan-rest-test-${crypto.randomUUID()}`,
                },
            });
...
afterAll(
    async () => {
        await testPrisma.restTracking.deleteMany({
            where: {
                tenantId:
                    testTenantId,
            },
...
describe(
    "Rest Tracking Repository Persistence",
    () => {

        it(
            "creates and retrieves a rest observation idempotently",
            async () => {
                ...
                if (created.kind !== "created") {
                    throw new Error(
                        "Expected rest tracking to be created.",
                    );
                }
                ...
                if (replay.kind !== "replayed") {
                    throw new Error(
                        "Expected identical rest observation to replay.",
                    );
                }
                ...
                await testPrisma.restTracking.delete({
                    where: {
                        id: tracking.id,
                    },
                });
            ...
            async () => {
                ...
                const otherTenant =
                    await testPrisma.tenant.create({
                        data: {
                            name:
                                `TITAN Other Rest Tenant ${crypto.randomUUID()}`,
                            slug:
                                `titan-other-rest-${crypto.randomUUID()}`,
                        },
                    });
