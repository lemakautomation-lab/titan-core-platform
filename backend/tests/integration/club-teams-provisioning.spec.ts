...
const code = "club.teams.read";

describe("Mission 070.8 new-tenant permission provisioning", () => {
    it("dry runs, rejects foreign roles and applies an idempotent audited grant", async () => {
        const tenant = await testPrisma.tenant.create({ data: {
            name: "070.8 provisioning gate", slug: `gate-0708-${randomUUID()}`,
        } });
        const foreignTenant = await testPrisma.tenant.create({ data: {
            name: "070.8 foreign tenant", slug: `gate-0708-${randomUUID()}`,
        } });
        try {
            ...
            const args = [
                "dist/scripts/provision-club-teams-permission.js",
            ...
            expect(await testPrisma.auditLog.count({ where: {
                tenantId: tenant.id, action: "CLUB_TEAMS_PERMISSION_PROVISION",
            } })).toBe(2);
        } finally {
