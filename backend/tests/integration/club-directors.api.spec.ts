...
const endpoint = "/api/v1/club/directors";
const permission = "club.directors.read";
...
describe("Mission 070.2 club performance director", () => {
    beforeEach(async () => { await rateLimitModule.resetAuthRateLimiter(); });

    it("requires authentication and the dedicated director permission", async () => {
    ...
    it("pages only explicitly assigned active directors in the actor's active club", async () => {
        const actor = await createTestUser({ permissions: [permission] });
        ...
        const role = await testPrisma.role.create({ data: { tenantId, name: "PERFORMANCE_DIRECTOR" } });
        ...
        expect(first.body).toEqual({ organisationId: own.id,
            directors: [{ id: ordered[0], name: "Named Executive" }], nextCursor: ordered[0],
        });
        const second = await fetchPage(first.body.nextCursor);
        expect(second.body.directors).toEqual([{ id: ordered[1], name: "Named Executive" }]);
