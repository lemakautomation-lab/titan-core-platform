...
const endpoint = "/api/v1/club/nutrition";
const permission = "club.nutrition.read";
...
describe("Mission 070.6 club nutrition", () => {
    beforeEach(async () => { await rateLimitModule.resetAuthRateLimiter(); });

    it("requires authentication and the dedicated nutrition permission", async () => {
    ...
    it("pages only explicitly assigned active professionals in the actor's active club", async () => {
        ...
        const role = await testPrisma.role.create({ data: { tenantId, name: "NUTRITION_PROFESSIONAL" } });
        const createNutrition = async (organisationId: string, status: "ACTIVE" | "INACTIVE" = "ACTIVE") => {
            const identity = await createTestUser({ tenantId });
            await testPrisma.user.update({ where: { id: identity.user.id },
                data: { organisationId, status, firstName: "Named", lastName: "Nutrition" },
            });
            await testPrisma.userRole.create({ data: { userId: identity.user.id, roleId: role.id } });
            return identity.user.id;
        };
        const firstId = await createNutrition(own.id);
        const secondId = await createNutrition(own.id);
        const otherId = await createNutrition(other.id);
        await createNutrition(own.id, "INACTIVE");
        ...
        expect(first.body).toEqual({ organisationId: own.id,
            nutrition: [{ id: ordered[0], name: "Named Nutrition" }], nextCursor: ordered[0],
        });
        const second = await fetchPage(first.body.nextCursor);
        expect(second.body.nutrition).toEqual([{ id: ordered[1], name: "Named Nutrition" }]);
