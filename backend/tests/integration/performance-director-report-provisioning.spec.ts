...
const code = "performance-director.reports.read";

describe("Mission 069.4 new-tenant permission provisioning", () => {
    it("dry runs, rejects foreign roles and applies an idempotent audited grant", async () => {
        const tenant = await testPrisma.tenant.create({ data: {
            name: "069.4 provisioning gate", slug: `gate-0694-${randomUUID()}`,
        } });
        const foreignTenant = await testPrisma.tenant.create({ data: {
            name: "069.4 foreign tenant", slug: `gate-0694-${randomUUID()}`,
        } });
        try {
            ...
            const args = [
                "dist/scripts/provision-director-report-permission.js",
            ...
            expect(await testPrisma.auditLog.count({ where: {
                tenantId: tenant.id, action: "DIRECTOR_REPORT_PERMISSION_PROVISION",
            } })).toBe(2);
        } finally {
