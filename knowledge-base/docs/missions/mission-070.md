...
- Added read-only `GET /api/v1/club/athletes` with bounded cursor pagination and an independent `club.athletes.read` permission. The reader resolves the authenticated actor's directly assigned active club and returns active Athletes with an explicit matching tenant and direct club assignment. Only names and opaque IDs are returned; no date of birth, contact details, measurements, goals, diagnoses or plans are exposed. Team membership and professional relationships do not infer club membership. Out-of-scope cursors and missing clubs fail closed.
- A tenant-scoped migration grants the new read permission to existing `ADMIN` roles; an audited dry-run-first operator command supports new tenants. Apply the migration before deploying the route. The Club page checks the organisation against its authorised executive view, loads Athletes only with the dedicated grant and handles errors without displaying foreign data. This roster does not grant access to Athlete records or health workflows.
- Added targeted API authentication, permission, input, pagination and cross-organisation/tenant scope tests; provisioning and frontend permission, pagination and mismatch tests. The migration applied to protected local titan_core_test. Focused backend tests passed (2/2 files, 4/4 tests); focused frontend and router tests passed (2/2 files, 43/43 tests). Full backend serial regression passed (211/211 files, 1357/1357 tests); full frontend regression passed (42/42 files, 246/246 tests). Backend and frontend builds, changed-file backend lint, frontend lint, Knowledge Base build and git diff --check passed. Commit, production rollout and Knowledge Base publication verification remain pending.
